...
int ip_run()
{
	...
	#endif
	{
		...
		while(ip_is_first_superstep() || ip_active_vertices > 0)
		{
			...
			// This barrier is crucial; it makes sure no thread goes to the spread vertices merge phase below, which uses ip_active_vertices, before ip_active_vertices has been incremented by all threads.
			#pragma omp barrier
