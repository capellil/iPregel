...
void ip_init_specific()
{
	...
	ip_edges_left_omp = (size_t*)ip_safe_malloc(sizeof(size_t) * ip_thread_count * IP_CACHE_LINE_LENGTH);
	for(int i = 0; i < ip_thread_count; i++)
	{
		ip_edges_left_omp[i * IP_CACHE_LINE_LENGTH] = 0;
	}
	ip_edges_left = 0;
	...
	for(int i = 0; i < ip_thread_count; i++)
	{
		...
		ip_all_spread_vertices_omp[i * IP_CACHE_LINE_LENGTH].data = ip_safe_malloc(sizeof(IP_VERTEX_ID_TYPE) * ip_all_spread_vertices_omp[i].max_size);
	}
}
